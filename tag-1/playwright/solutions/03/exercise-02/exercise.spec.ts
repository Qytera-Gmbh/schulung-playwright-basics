...
test.describe("Create invitation", () => {
  ...
  test("[03][02] Create invitation", async ({ page }) => {
    ...
    await invitationCard
      .getInputComments()
      .fill("I am looking forward to seeing you!");
    ...
    await Promise.all([
      ...
      expect
        .soft(modal.getHostMail())
        .toHaveText("Contact: john.doe@example.org"),
      expect
        .soft(modal.getLocation())
        .toHaveText("Location: Big City, Roosevelt Road 52"),
      expect
        .soft(modal.getDresscodeDescription())
        .toHaveText(
          "Come dressed in our theme colors: #ffaa00 and #0033ff! Show off your creativity while rocking these colors to add to the festive vibe."
        ),
      expect
        .soft(modal.getAdditionalComments())
        .toHaveText("I am looking forward to seeing you!"),
    ]);
  });
});
